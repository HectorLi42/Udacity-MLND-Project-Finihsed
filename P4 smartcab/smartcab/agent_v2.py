...
class LearningAgent(Agent):
    ...
    def Alpha_decay(self,CurrentCounter,valid = True):
        if valid:
            return self.alpha/float(CurrentCounter)
        else:
            return self.alpha
        
    def Epsilon_decay(self,CurrentCounter,valid = True):
        if valid:
    ...
    def update(self, t):
        ...
        # create next state
        NextState = OrderedDict()
        NextState['guide'] = self.next_waypoint
        NextState['light'] = inputs['light']
        NextState['oncoming'] = inputs['oncoming']
        NextState['left'] = inputs['left']
        NextState['right'] = inputs['right']
        #NextState['deadline'] = deadline
        NextState = tuple(NextState.items()) # use this tuple to represent possible state
        
        if not(self.StateCounter.has_key(NextState)):
            #self.Qtable[NextState] = np.zeros(4)
            self.Qtable[NextState] = np.random.rand(4)
            self.StateCounter[NextState] = 0
            
        alpha_tmp = self.Alpha_decay(CurrentCounter,valid = self.decayValid)
        self.Qtable[CurrentState][RealDecision] = (1-alpha_tmp)*self.Qtable[CurrentState][RealDecision] + alpha_tmp * (reward + self.gamma*self.Qtable[NextState].max())
